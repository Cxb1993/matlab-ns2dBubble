function m = testCirc(m,a,a1,problem)
...
for ncirc=1:ncircmax
    ...
    for jj=1:(n_lados_primeiro_poligono*ncirc)
        Xcirc(j,1)=a*cos(theta);
        Ycirc(j,1)=b*sin(theta);
        theta = theta+dtheta;
        j=j+1;
        if (theta >= 2*pi)
            break;
...
xc=(max(m.X)+min(m.X))/2;
yc=(max(m.Y)+min(m.Y))/2;


np=3*a;
rb = a1/5;
rb=round((max(m.X)-xc)/4);
%xc = (a-1)/2;
%yc = (a1-1)/2;
exc=1;
% for i=1:np
%     theta = 2*pi*(i-1)/np;
%     m.X=[m.X;xc+rb*cos(theta)];
%     m.Y=[m.Y;yc+exc*rb*sin(theta)];
% end;
m.Z = 0*m.X;
...
if(strcmp(problem,'cavityBubble'))
...
%     end;
    ...
    for i=1:nvert
        ...
        if( ((X(i)-xc)*(X(i)-xc)) + ((Y(i)-yc)*(Y(i)-yc))/exc/exc < rb*rb )
            cc(i)=1;
        end;
        ...
        if( ((X(i)-xc)*(X(i)-xc)) + ((Y(i)-yc)*(Y(i)-yc))/exc/exc > rb*rb )
         ...
         if( abs(((X(i)-xc)*(X(i)-xc)) + ((Y(i)-yc)*(Y(i)-yc))/exc/exc - rb*rb) < 10e-2 )
            cc(i)=0.5;
        end;
    end;
end;
